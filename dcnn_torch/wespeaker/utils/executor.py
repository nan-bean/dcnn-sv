...
def run_epoch(dataloader,
    ...
    with torch.set_grad_enabled(True), model_context():
        for i, batch in enumerate(dataloader):
            ...
            l2_reg = l2_reg + torch.square(torch.norm(model.state_dict()["module.seg_1.weight"]))
            l2_reg = l2_reg + torch.square(torch.norm(model.state_dict()["module.seg_1.bias"]))
            loss = loss + 0.00001 * l2_reg
