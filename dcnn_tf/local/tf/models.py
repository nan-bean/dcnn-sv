...
import kaldi_io
from tf_block import batch_norm_wrapper, residual_block_dykcnn_v2, conv2d_bn_layer_v2, residual_block_v2, AM_logits_compute_use_onehot
...
class ResNet34_dcnn(Model):

    def __init__(self):
        super(ResNet34_dcnn, self).__init__()
        self.blocks = [3,4,6,3]
        self.channels = [32,64,128,256]


    def resnet_conv2d(self,h):
        with tf.variable_scope("resnet_conv2d_layer"):
            with tf.variable_scope("conv1"):
                h = conv2d_bn_layer_v2(h,[7,7,1,self.channels[0]],[1,2,2,1],self.phase)
                h = tf.nn.relu(h)
                
            for i in range(self.blocks[0]):
                with tf.variable_scope("conv2_%d" % i):
                    h = residual_block_dykcnn_v2(h, self.channels[0], self.phase, down_sample=False, projection=True)
            
            for i in range(self.blocks[1]):
                with tf.variable_scope("conv3_%d" % i):
                    if i==0:
                        h = residual_block_dykcnn_v2(h, self.channels[1], self.phase, down_sample=True, projection=True)
                    else:
                        h = residual_block_dykcnn_v2(h, self.channels[1], self.phase, down_sample=False, projection=True)
            for i in range(self.blocks[2]):
                with tf.variable_scope("conv4_%d" % i):
                    if i==0:
                        h = residual_block_dykcnn_v2(h, self.channels[2], self.phase, down_sample=True, projection=True)
                    else:
                        h = residual_block_dykcnn_v2(h, self.channels[2], self.phase, down_sample=False, projection=True)
            for i in range(self.blocks[3]):
                with tf.variable_scope("conv5_%d" % i):
                    if i==0:
                        h = residual_block_v2(h, self.channels[3], self.phase, down_sample=True, projection=True)
                    else:
                        h = residual_block_v2(h, self.channels[3], self.phase, down_sample=False, projection=True)
        return h
    ...
    def build_model(self, num_classes, input_feature_dim, output_dir, logger=None):
        ...
        with self.graph.as_default():
            ...
            # Frame level layer: resnet-34
            h = self.resnet_conv2d(h)
            print("output tensor of resnet34_conv2d: ")
            print(h)
            ...
            prev_dim = h.get_shape().as_list()[-1]
            print("output tensor of the pooling layer: ")
            print(h)
            ...
            # AM-Softmax
            with tf.variable_scope("output"):
                scores = AM_logits_compute_use_onehot(h,self.input_y,prev_dim,num_classes,is_training=self.phase,s=30,m=0.15)
